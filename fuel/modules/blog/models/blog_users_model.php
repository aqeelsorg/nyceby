...
class Blog_users_model extends Base_module_model {
	...
	{
		$this->db->select('fuel_user_id, CONCAT(first_name, " ", last_name) as name, display_name, fuel_blog_users.active,fuel_blog_users.avatar_image', FALSE);
	...
	function options_list($key = 'fuel_user_id', $val = 'display_name', $where = array(), $order = 'display_name')
	{
		if ($key == 'id')
		{
			$key = $this->table_name.'.fuel_user_id,'.$this->table_name.'avatar_image';
		}
		if ($val == 'display_name')
		{
	...
	function get_author_avatar_image($id)
	{
		$this->db->select($this->_tables['blog_users'].'.*');
		$this->db->join('fuel_blog_posts', 'fuel_blog_users.fuel_user_id = fuel_blog_posts.author_id', 'left');
		$this->db->where('id',$id);
		
		$result	= $this->db->get($this->_tables['blog_users']);
