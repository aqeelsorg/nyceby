...
class Blog_posts_model extends Base_module_model {
	...
	function addHitCount()
	{
		...
		$postDetails = $this->find_one(array("id"=>(int)$uri));
		
		$hitCount = $postDetails->get_hits() + 1;

		$data = array("hits"=>$hitCount);
		
		$this->db->where('id', $uri);
		
		$this->db->update($this->_tables['blog_posts'], $data); 
	}		
	
	function getPopularPosts()
	{
		$this->db->select($this->_tables['blog_posts'].'.*');
		$this->db->where('hits >=',10);
		$this->db->order_by('hits','DESC');
		
		$result	= $this->db->get($this->_tables['blog_posts']);
