...
class Blog extends Blog_base_controller {
	...
	function _remap()
	{
		...
		//	var_dump($cache);exit;
			...
			if($this->uri->rsegment(2) != "post" && $this->uri->rsegment(2) != "index")
			{
				$view = $this->uri->rsegment(2);
				$output = $this->_render($view, $vars, TRUE,'blogwithus');
	...
	function blogwithus()
	{
		...
		$output = $this->_render('blogwithus', $vars, TRUE);
	...
	function post($permalink = null)
	{
		...
		if(empty($_POST))
		{
			...
			$this->Blog_posts_model->addHitCount();
		}
