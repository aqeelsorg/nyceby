...
require_once(MODULES_PATH.'/site/config/site_constant.php');


Class Product_model extends Base_module_model
{		
	...
	{
		parent::__construct('pt_products',SITE_FOLDER);
	...
	function products($data=array(), $return_count=false)
	{
		...
		else
		{
			...
			{
				 var_dump($this->db->get('products')->result());exit;
        ...
        function get_all_products_byid()
        {
            ...
            $sql="SELECT pt_products.id, pt_products.sku, pt_products.name, pt_products.slug, pt_products.route_id, pt_products.description, pt_products.excerpt, pt_products.price, pt_products.saleprice, pt_products.free_shipping, pt_products.shippable, pt_products.taxable, pt_products.fixed_quantity, pt_products.weight, pt_products.track_stock, pt_products.quantity, pt_products.related_products, pt_products.images, pt_products.seo_title, pt_products.meta, pt_products.enabled, pt_products.author_id, pt_products.s_id,pt_products.hit_cnt, pt_buyer.image, pt_wishlist.u_id,pt_wishlist.p_id
                    FROM pt_products
                    JOIN pt_wishlist ON pt_products.id = pt_wishlist.p_id
                    JOIN pt_user ON pt_user.id = pt_wishlist.u_id
                    JOIN pt_buyer ON pt_buyer.b_id = pt_user.id
...
INNER JOIN pt_seller ON pt_user.id = pt_seller.s_id
...
ORDER BY pt_products.id ASC";
	...
	function get_cart_ready_product($id, $quantity=false)
	{
	   ...
	   $this->db->update('products', $data);
        ...
        {
             $sql="SELECT pt_products.id, pt_products.hit_cnt AS pcnt, pt_products.name,pt_products.created_on,pt_products.hit_cnt,pt_buyer.image
                    FROM pt_products
                    INNER JOIN pt_user ON pt_user.id = pt_products.s_id
                    JOIN pt_wishlist ON pt_user.id = pt_wishlist.u_id
                    JOIN pt_buyer ON pt_buyer.b_id = pt_wishlist.u_id
        ...
        function get_all_popuar_tumblr_products()
        {
                $buyer_info = $this->session->userdata('buyer');
                $b_id=$buyer_info['id'];
                 $sql="SELECT pt_tumblr_posts.blog_name, pt_tumblr_posts.t_id, pt_tumblr_posts.post_url, pt_tumblr_posts.slug, pt_tumblr_posts.type, pt_tumblr_posts.date, pt_tumblr_posts.created_on, pt_tumblr_posts.timestamp, pt_tumblr_posts.state, pt_tumblr_posts.format, pt_tumblr_posts.reblog_key, pt_tumblr_posts.tags, pt_tumblr_posts.body, pt_tumblr_posts.short_url, pt_tumblr_posts.highlighted, pt_tumblr_posts.bookmarklet, pt_tumblr_posts.note_count, pt_tumblr_posts.title, pt_tumblr_posts.url, pt_tumblr_posts.description, pt_tumblr_posts.caption, pt_tumblr_posts.text, pt_tumblr_posts.source, pt_tumblr_posts.photo, pt_tumblr_posts.u_id, pt_buyer.image, pt_user.username 
FROM pt_tumblr_posts
INNER JOIN pt_buyer ON pt_buyer.b_id=pt_tumblr_posts.u_id 
INNER JOIN pt_wishlist ON pt_buyer.b_id = pt_wishlist.u_id 
INNER JOIN pt_user ON pt_buyer.b_id = pt_user.id 
WHERE pt_buyer.b_id = $b_id";
                $q = $this->db->query($sql);
                $return = $q->result();
                return $return;
               // var_dump($return);exit;
