...
<table class="addtocart_table">
		<thead>
			<thead>
				<tr>
					...
					<th style="width:10%;text-align: left;"><?php echo lang('quantity');?></th>
		...
		<tfoot>
			...
			**************************************************************/
        	...
        	<tr>
				<td colspan="5"><strong style="padding-left: 13px;"><?php echo lang('group_discount');?></strong></td>
				<td><?php echo format_currency(0-$this->go_cart->group_discount()); ?>                </td>
			</tr>
			<?php endif; ?>
			<tr>
		    	<td colspan="5"><strong style="padding-left: 13px;"><?php echo lang('subtotal');?></strong></td>
			...
			**************************************************************/
			?>
			<tr>
				<td colspan="5"><strong style="padding-left:13px;"><?php echo lang('grand_total');?></strong></td>
		...
		<tbody>
			...
			foreach ($this->go_cart->contents() as $cartkey=>$product):?>
				<tr style="height: 70px;">
					...
					<td style="padding-left: 24px;"><?php echo format_currency($product['price']);?></td>
					<td style="width: 36%;">
								...
								<div class="control-group">
									<div class="controls">
										<div class="input-append">
											<input class="span1" style="margin:0px;" name="cartkey[<?php echo $cartkey;?>]"  value="<?php echo $product['quantity'] ?>" size="3" type="text"><button class="btn btn-danger" type="button" onclick="if(confirm('<?php echo lang('remove_item');?>')){window.location='<?php echo site_url('site/remove_item/'.$cartkey);?>';}"><i class="icon-remove icon-white"></i></button>
										</div>
									</div>
								</div>
							<?php else:?>
								...
								<button class="btn btn-danger" type="button" onclick="if(confirm('<?php echo lang('remove_item');?>')){window.location='<?php echo site_url('site/remove_item/'.$cartkey);?>';}"><i class="icon-remove icon-white"></i></button>
							<?php endif;?>
						<?php else: ?>
							<?php echo $product['quantity'] ?>
						<?php endif;?>
